...
This document describes the augmented CircuitJSON arrays and renderer models
retained by the 1.0.29 native extension API.
...
The canonical projection distinguishes actual `schematic_sheet` page rows
from hierarchical `schematic_sheet_symbol` child boxes, preserves each child
file as `source_file_name`, and links child ports through
`schematic_sheet_symbol_id`. Embedded image bytes move to
`document.assets` as `schematic-image` ToolkitAssets and are referenced by
canonical `schematic_image.asset_id`.
...
radius, local offset, solder-mask coverage and margin, port layer, and component
owner. Face offsets rotate with the source pad and are folded into the canonical
pad and port center because CircuitJSON pads do not carry a separate offset
field. A bottom-only pad therefore never falls back to an absent top-face size
or to unscaled raw KiCad millimeter dimensions.
