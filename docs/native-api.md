...
This document covers the complete 1.0.29 API retained at
`kicad-toolkit/extensions` and its explicit extension subpaths.

## Entrypoints

`kicad-toolkit/extensions` exports the supported parser, renderer, and 3D
scene-description classes from one browser-safe entrypoint.

Specialized entrypoints are also available:

- `kicad-toolkit/extensions`
- `kicad-toolkit/extensions/netlist-query`
- `kicad-toolkit/extensions/node`
...
```js
import { CircuitJsonModelSchema } from 'kicad-toolkit/extensions'
...
`kicad-toolkit/extensions/node` as an optional host-gated KiCad CLI snapshot
helper. It is
...
See [Native Capabilities](native-capabilities.md) for the full inventory and
report shapes.

## Netlist Query

```js
import { LoadedDesignNetlistService } from 'kicad-toolkit/extensions/netlist-query'
